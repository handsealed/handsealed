 ...
 *
 * This is the root of the dependency graph — the engine (which asks the
 * questions) and every backend that answers them (`@handsealed/facts-git`
 * today; a sidecar or forge-API adapter later) depend on this and never on
 * each other. It carries no runtime code and no dependencies: just the
 * contract and its git-shaped value types.
 *
